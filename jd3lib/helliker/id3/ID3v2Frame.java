...
public class ID3v2Frame {
  ...
  public byte[] getFrameBytes() {
    ...
    System.arraycopy(BinaryParser.convertToBytes(length), 0, b, bytesWritten, 4);
    bytesWritten += 4;
  ...
  public int getDataLength() {
    ...
    /** 
     *@returns ID3v2Comment with the comment to this Mp3file, null if frame not set
     */
    public ID3v2Comment getComment() throws ID3v2FormatException {
        if (!id.equals(ID3v2Frames.COMMENTS)) {
            throw new RuntimeException("This is not a comment frame but: "  + id);
        }
        
        if (frameData.length > 1) {
            ID3v2Comment id3v2Comment = new ID3v2Comment();
            try {            
                int encType = (int) frameData[0];
                byte[] language = new byte[3];
                language[0] = frameData[1];
                language[1] = frameData[2];
                language[2] = frameData[3];
                id3v2Comment.setLanguage(new String(language));
                byte[] description = new byte[frameData.length]; //to long but we correct that later
                
                boolean done = false;
                int i = 0;
                while (!done && (i < description.length)) {
                    done = (frameData[i + 4] == '\0');  //byte 0 is seperator beteen decription and actual text
                    if (!done) {
                        description[i] = frameData[i + 4];
                        i++;
                    }
                }            
                if ((encType >= 0) && (encType < ENC_TYPES.length)) {
                    id3v2Comment.setDescription(new String(description, 0, i, ENC_TYPES[encType])); //i indicates the end of the description
                }            
                id3v2Comment.setText(new String(frameData, i + 5, frameData.length - (i+5), ENC_TYPES[encType]));            
    ...
    /** 
     *@returns ID3v2UserDefinedURL with the UserDefinedURL of this Mp3file, null if frame not set
     */
    public ID3v2UserDefinedURL getUserDefinedURL() throws ID3v2FormatException {
        if (!id.equals(ID3v2Frames.USER_DEFINED_URL)) {
            throw new RuntimeException("This is not a USER_DEFINED_URL frame but: "  + id);
        }
        
        if (frameData.length > 1) {
            ID3v2UserDefinedURL id3v2UserDefinedURL = new ID3v2UserDefinedURL();
            try {            
                int encType = (int) frameData[0];                
                byte[] description = new byte[frameData.length]; //to long but we correct that later                
                boolean done = false;
                int i = 0;
                while (!done && (i < description.length)) {
                    done = (frameData[i] == '\0');  //byte 0 is seperator beteen decription and actual url
                    if (!done) {
                        description[i] = frameData[i];
                        i++;
                    }
                }            
                if ((encType >= 0) && (encType < ENC_TYPES.length)) {
                    id3v2UserDefinedURL.setDescription(new String(description, 0, i, ENC_TYPES[encType])); //i indicates the end of the description
                }            
                id3v2UserDefinedURL.setURL(new String(frameData, i + 2, frameData.length - (i+2), ENC_TYPES[encType]));            
            } catch (java.io.UnsupportedEncodingException e) {
                throw new ID3v2FormatException("Frame: " + id + " has "
                                    + " not specified a valid encoding type.");
            }
            return id3v2UserDefinedURL;
        } else {
            return null;
        }
  ...
  private String getDecodedString(byte[] b, int eIndex, int offset)
    ...
    if ((encType >= 0) && (encType < ENC_TYPES.length)) {
      str = new String(b, offset, b.length - offset, ENC_TYPES[encType]);
    }
    return str;
  }
