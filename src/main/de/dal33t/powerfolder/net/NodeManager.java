...
public class NodeManager extends PFComponent {
    ...
    public void setNetworkId(String networkId) {
        ...
        if (Constants.NETWORK_ID_ANY.equals(networkId)) {
            logFine("Changing network ID to ANY for federation sync");
        }
        ...
        if (friend) {
                ...
                && !getMySelf().isServer())
            {
                getController().getTaskManager()
                    .scheduleTask(new SendMessageTask(new AddFriendNotification(
                        mySelf.getInfo(), personalMessage), node.getId()));
            }
        } else if (wasFriend) {
