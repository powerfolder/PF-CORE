...
public class CompositeMap<K,V> implements Map<K,V> {
     ...
     */
    public CompositeMap(MapMutator mutator, Map<K,V>... composite) {
    ...
    {
        for (int i = composite.length - 1; i >= 0; --i) {
            Collection intersect = intersection(this.composite[i].keySet(),
                map.keySet());
            if (intersect.size() != 0) {
     ...
     *             (optional).
     * @throws NullPointerException
     *             if the value is <tt>null</tt> and this map does not not
     ...
     * the returned set is a <code>Map.Entry</code>. The set is backed by the
     ...
     *             it from being stored in this map.
     * @throws NullPointerException
     *             the specified map is <tt>null</tt>, or if this map does not
     *             permit <tt>null</tt> keys or values, and the specified map
     ...
     *             (optional).
     * @throws NullPointerException
     *             if the key is <tt>null</tt> and the composited map does not
     *             not permit <tt>null</tt> keys (optional).
     * @throws UnsupportedOperationException
     *             if the <tt>remove</tt> method is not supported by the
    ...
    /**
     * This interface allows definition for all of the indeterminate mutators in
     * a CompositeMap, as well as providing a hook for callbacks on key
     * collisions.
     */
    public static interface MapMutator {
         ...
         *            the Map being added
         * @param intersect
         *            the intersection of the keysets of the existing and added
         *            maps
         */
