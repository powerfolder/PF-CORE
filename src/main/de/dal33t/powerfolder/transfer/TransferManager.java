...
public class TransferManager extends PFComponent {
    ...
    void setCompleted(final DownloadManager dlManager) {
        ...
        autoClean = autoClean
            ...
            || (!PreferencesEntry.EXPERT_MODE.getValueBoolean(getController()) && !ConfigurationEntry.DOWNLOAD_AUTO_CLEANUP_FREQUENCY
    ...
    private boolean requestDownload(Download download, Member from) {
        ...
        synchronized (dlManagers) {
                ...
                || !fInfo.isVersionDateAndSizeIdentical(man.getFileInfo()))
            {
                if (man != null) {
                    if (!man.isDone()) {
                        if (isFine()) {
                            logFine("Aborting download. Got active download of different file version: " + man);
                        }
                        man.abortAndCleanup();
                    }
                    return false;
        ...
        synchronized (man) {
                ...
                getController().getFolderRepository()).getFile(fInfo)))
            {
                // Skip exact same version etc.
                if (isFine()) {
                    logFine("Aborting download, already have latest file version: "
                            + fInfo.toDetailString());
                }
                man.abort();
                return false;
