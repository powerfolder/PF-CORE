...
public class PriorityTransferTest extends TwoControllerTestCase {
    ...
    public void testPriorityRequests() {
        ...
        final FileInfo[] fInfos = getFolderAtLisa().getKnowFilesAsArray();
        ...
        assertTrue(prio.getComparator().compare(
            fInfos[0], fInfos[1]) > 0);
        
        mockery.checking(new Expectations() {{
            Sequence dlSeq = mockery.sequence("Sequence of requests");
            one(helperBart).downloadNewestVersion(fInfos[1], true); inSequence(dlSeq);
            one(helperBart).downloadNewestVersion(fInfos[2], true); inSequence(dlSeq);
